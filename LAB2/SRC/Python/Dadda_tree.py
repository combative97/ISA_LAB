...
#for i in range (first_level):
#   for j in range (2*nb):
#      matrix_file.write(partial_p[i][j])
#       matrix_file.write('\n')
...
#    out_file = 'DADDA_VHDL_LEVEL_' + str(file_iter+1) + '.txt'
#    pj_file = 'partial_product_' + str(file_iter+1) + '.txt'
#    product_file = open(pj_file,'w')
    ...
    shift = 0
    partial_iter = 0
#    for r in range (levels[num_levels-2]):
#        for c in range (2*nb):
#                product_file.write(result[r][c])
#        product_file.write('\n')
output_file.close()
#    product_file.close()
